...
class LineChartPainter extends AxisChartPainter<LineChartData> {
  ...
  void drawBar(
    ...
    // // canvasWrapper.drawDot(painter, spot, offset)
    if (!barData.dotData.show || barData.spots.isEmpty) {
      return;
    }
    final viewSize = canvasWrapper.size;
    ...
    if (spot.isNotNull() && barData.dotData.checkToShowDot(spot, barData)) {
      ...
      // print(canvasWrapper.size.width);
      // print(dotOffset);
      final y2 = getYFromX(
        canvasWrapper,
        barPath,
      ...
      canvasWrapper.canvas.drawLine(
        Offset(canvasWrapper.size.width / 2, y2),
        Offset(canvasWrapper.size.width / 2 + 1000, y2),
      ...
      canvasWrapper.drawDot(
          ...
          spot,
          Offset(
            // dotOffset * canvasWrapper.size.width / 2 + canvasWrapper.size.width,
            dotOffset * canvasWrapper.size.width / 2 +
  ...
  double getYFromX(CanvasWrapper canvasWrapper, Path path, double x) {
    final metrics = path.computeMetrics();
    for (final metric in metrics) {
      // return metric.getTangentForOffset(x)?.position.dy ?? -1;
      // if (segment.contains(Offset(x, 0))) {
      // print('x: $x');
      final widthPoint = canvasWrapper.size.width / 2;
      return metric.getTangentForOffset(metric.length/1.45)?.position.dy ?? -1;

      //   final y = segment.pointAlongPath(metric.length / 2).dy;
      //   return y;
      // }
    }
    return 0;
